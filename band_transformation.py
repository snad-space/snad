...
def band_transformation(msd, a, b, new_bands, old_bands=None):
    ...
    for i, x in enumerate(msd.odict[old_bands[0]].x):
        ...
        if np.any(old_flux <= 0):
            new_flux = np.full_like(b, np.nan)
        else:
...
def VR_to_gri(msd):
    ...
    new_bands = ('g', 'r', 'i')
    old_bands = ('V', 'R')
    return band_transformation(msd, a, b, new_bands, old_bands=old_bands)
